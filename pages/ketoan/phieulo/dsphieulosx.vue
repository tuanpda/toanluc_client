                ...
                <div class="columns">
                    <div class="column is-11">
                        <div class="control">
                            ...
                            <span style="color: #3850b7; font-size: 17px; font-weight: bold">Danh sách phiếu lô sản
                                xuất</span>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        chotphieu(phieulosx) {
            this.isActive = true;
            this.getinfoplsx = phieulosx;
            // console.log(phieulosx)
        },

        chonngaychot() {
            if (this.form.stopday_losx != "") {
        ...
        async xacnhanChotphieu(phieu) {
            let data = {
                stopday_losx: this.form.stopday_losx,
                status: 1,
                tongdat: this.form.tongdat,
                tonghong: this.form.tonghong,
            };
            await this.$axios.$patch(
                `/api/ketoan/capnhatstatuslcd?makh=${this.form.makh}&makhpx=${this.form.makhpx}&malosx=${this.form.malosx}&mapx=${this.form.mapx}`,
                data
            );
            await this.$axios.$patch(
                `/api/ketoan/capnhatstatuslosx?makh=${this.form.makh}&makhpx=${this.form.makhpx}&malosx=${this.form.malosx}&mapx=${this.form.mapx}`,
                data
            );
            // console.log(phieu);
        ...
        async getInfoLosx(infoPhieulo) {
            ...
            // load cong nhan mặc định
            // let ispxorto;
            // ispxorto = await this.$axios.$get(
            //   `/api/ketoan/toornhom?mapx=${this.getinfoplsx.mapx}`
            // );
            // if (ispxorto.length > 0) {
            //   this.cong_nhan = await this.$axios.$get(
            ...
            //   );
            // }
            // console.log(this.getinfoplsx.mato);
            // if (this.getinfoplsx.mato == null) {
            ...
            for (let i = 0; i < this.items_cn.length; i++) {
                if (i == index) {
                    ...
                    this.items_cn[i].nguoithuchien =
                        this.cong_nhan[this.selectedIndex].tencn;
        ...
        async onDelete(lsx) {
            let ds = []
            ds = await this.$axios.$get(
                `/api/lokehoach/checkcongdoaninlsx?makh=${lsx.makh}&makhpx=${lsx.makhpx}&mapx=${lsx.mapx}&malosx=${lsx.malosx}`
            );
            // console.log(ds)
            if (ds.length > 0) {
                const Toast = Swal.mixin({
                ...
                Toast.fire({
                    icon: "error",
                    title: "Xóa toàn bộ công đoạn trong lô sản xuất trước khi xóa lô sản xuất !!!",
                });
            } else {
                swal({
                    ...
                    if (willDelete) {
                        this.$axios.$delete(`/api/lokehoach/losx/${lsx._id}`)
                            .then(response => {
                                const index = this.showlsxpx.findIndex(lo => lo._id === lsx._id) // find the post index 
                                if (~index) // if the post exists in array
                                    this.showlsxpx.splice(index, 1) //delete the post
                            });
